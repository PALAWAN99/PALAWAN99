...
import {
  Title, Text, Stack, Group, Button, Card, Tabs, Table, Badge, ActionIcon, Modal, TextInput, NumberInput, Select, Switch, Center, Paper, SimpleGrid, Divider,
} from '@mantine/core';
import {
  IconQrcode, IconSettings, IconPlus, IconEdit, IconSearch, IconUserCheck, IconClock, IconRepeat,
...
export default function QRManagementPage() {
  ...
  const handleMemberSearch = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data && !data.error) setMemberData(data);
  ...
  const handleIssueQR = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        notifications.show({ title: 'Error', message: result.error?.message || 'Issuance failed', color: 'red' });
      }
    } catch (error) {
