...
// ตรวจสอบความปลอดภัยของ QR_SECRET ทันทีเมื่อโหลดโมดูลนี้
const qrSecret = env.QR_SECRET;

if (!qrSecret || qrSecret.length < 32) {
  throw new Error(
    `\n❌ [Security Error] ตั้งค่า QR_SECRET ไม่ถูกต้อง หรือมีความปลอดภัยไม่เพียงพอ!\n` +
    `👉 ปัญหา: QR_SECRET ต้องไม่ว่างเปล่า และต้องมีความยาวอย่างน้อย 32 ตัวอักษร เพื่อป้องกันการโจมตีประเภท HMAC forging\n` +
    `🛠️ วิธีแก้ไข: กรุณากำหนดค่า 'QR_SECRET' ที่มีความปลอดภัยสูงในไฟล์ '.env' ที่บริเวณ root ของ repository\n`
  );
}
...
export function generateToken(memberId: string) {
  ...
  // สร้าง Signature (HMAC) เพื่อป้องกันการปลอมแปลง (ใช้ SHA-256 เต็ม 64 hex chars / 256-bit)
